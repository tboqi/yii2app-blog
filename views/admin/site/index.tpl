...
<aside class="Hui-aside">
  <div class="menu_dropdown bk_2">
    {{foreach $menu as $_m}}
    <dl id="menu-article">
      <dt><i class="Hui-iconfont">&#xe616;</i> {{$_m.name}}<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>
      <dd>
        {{if $_m._child}}
        <ul>
          {{foreach $_m._child as $_m1}}
          <li><a data-href="{{$_m1.route}}" data-title="{{$_m1.name}}" href="javascript:void(0)">{{$_m1.name}}</a></li>
          {{/foreach}}
        </ul>
        {{/if}}
      </dd>
